/*! Wireframe Kit runtime v1.2.3 — 表紙・コメント・PNG書き出し */
(function(){
...
function wfJump(id,smooth){
  ...
  // ページ上端の区切り線（border-top）はヘッダーの下に見せない：線の太さぶん下に合わせる
  const bt=parseFloat(getComputedStyle(el).borderTopWidth)||0;
  const top=el.getBoundingClientRect().top+window.scrollY-pad+bt;
