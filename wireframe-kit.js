/*! Wireframe Kit runtime v1.0.0 — 表紙・コメント・PNG書き出し */
(function(){
...
function load(cb){try{const d=JSON.parse(LS.getItem(STORE)||'[]');if(Array.isArray(d)){comments=d;nextId=d.length?Math.max(...d.map(c=>c.id))+1:1;}}catch(e){}if(cb)cb();}
function save(){try{LS.setItem(STORE,JSON.stringify(comments));}catch(e){alert('ブラウザの保存容量がいっぱいです。画像付きのコメントを減らすか、「コメント書き出し」でファイルに保存してください。');}}
...
function wfFocus(id){wfTogglePop(id);}
window.wfFocus=wfFocus;
function wfFileBase(){return (document.title||'wireframe').replace(/[\\\/:*?"<>|\s]+/g,'_');}
function wfExportComments(){
  const a=document.createElement('a');
  a.href=URL.createObjectURL(new Blob([JSON.stringify({id:WF_ID,exportedAt:new Date().toISOString(),comments:comments})],{type:'application/json'}));
  a.download=wfFileBase()+'_コメント.json';document.body.appendChild(a);a.click();a.remove();
}
window.wfExportComments=wfExportComments;
function wfImportComments(inp){
  const f=inp.files[0];if(!f)return;
  const r=new FileReader();
  r.onload=e=>{
    try{
      const d=JSON.parse(e.target.result);const list=Array.isArray(d)?d:(d.comments||[]);let added=0;
      const same=(a,b)=>a.author===b.author&&a.time===b.time&&a.text===b.text;
      list.forEach(c=>{
        const hit=comments.find(x=>same(x,c));
        if(hit){(c.replies||[]).forEach(rp=>{hit.replies=hit.replies||[];if(!hit.replies.some(y=>same(y,rp))){hit.replies.push(rp);added++;}});if(c.resolved)hit.resolved=true;}
        else{comments.push(Object.assign({},c,{id:nextId++}));added++;}
      });
      save();renderPins();renderSidebar();alert(added+'件のコメント・返信を読み込みました');
    }catch(err){alert('読み込めませんでした。「コメント書き出し」で保存したファイル（.json）を選んでください');}
    inp.value='';
  };
  r.readAsText(f);
}
window.wfImportComments=wfImportComments;
